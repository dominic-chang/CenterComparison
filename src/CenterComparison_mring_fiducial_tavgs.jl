...
for file_name in readdir(joinpath(dirname(@__DIR__), "data", "tavgs_fits_480")) |> fn1
    ...
    # Create Fiducial Ring Model
    function floored_mring(params)
        (;rad, width, rot, α1, β1, α2, β2, ell, floor, x, y) = params
        flatfloor = CM.modify(
            Disk(),
        ...
        model = CM.modify(
            CM.smoothed(floor*mring + (1-floor)*flatfloor,CM.μas2rad(width) / (2 * √(2 * log(2)))),
            CM.Rotate(rot),
        )
    ...
    prob = VIDA.VIDAProblem(bh, floored_mring, lower_bound, upper_bound)
    ...
    # Save results
    outpath = joinpath((@__DIR__), "..", "results", "mring_fiducial")
    try
        mkdir(outpath)
    catch e
        println(e)
